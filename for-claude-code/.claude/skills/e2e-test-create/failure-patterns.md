...
| 漢字2文字ラベル（「検索」等）のテキスト完全一致 | ❌ | antd の autoInsertSpace で DOM は「検 索」。text-is / has-text / role+name+exact 全滅。アンカー付き正規表現 `{ name: /^検\s*索$/ }` のみ通る（アンカーを省くと「再検索」に誤爆）。アイコン付き・`type="text"`/`"link"` では発動しない |
